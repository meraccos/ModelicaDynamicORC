package Rankine
  ...
  package Tests
    model Test_StaticHX
      ...
      Components.StaticHX staticHX(h_t = 8000, hc_in_nom = 250e3, hh_in_nom = 2.8e6, n = 50, p_cold(displayUnit = "Pa"), p_cold_nom(displayUnit = "Pa") = 1e5, p_hot(displayUnit = "Pa"), p_hot_nom(displayUnit = "Pa") = 1e5) annotation(
    ...
    model Test_QuasiStaticHX
      ...
      Components.StaticHX staticHX(h_t = 8000, hc_in_nom = 250e3, hh_in_nom = 2.8e6, n = 25, p_cold(displayUnit = "Pa"), p_cold_nom(displayUnit = "Pa") = 1e5, p_hot(displayUnit = "Pa"), p_hot_nom(displayUnit = "Pa") = 1e5) annotation(
        Placement(visible = true, transformation(origin = {-10, 4}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
    ...
    model Test_DynamicHX
      ...
      ThermoPower.Water.SinkPressure sinkPressure annotation(
        Placement(visible = true, transformation(origin = {46, 26}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      ThermoPower.Water.SourceMassFlow sourceMassFlow1(h = 3.2e6, use_T = false, w0 = 0.5) annotation(
        Placement(visible = true, transformation(origin = {54, -16}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
      inner ThermoPower.System system annotation(
        Placement(visible = true, transformation(origin = {86, 86}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.DynamicHX dynamicHX(Q_nom = 1, h_t = 8000, hc_in_nom = 150e3, hh_in_nom = 2.8e6, n = 30, p_cold(displayUnit = "Pa"), p_cold_nom(displayUnit = "Pa") = 99999.99999999999, p_hot(displayUnit = "Pa"), p_hot_nom(displayUnit = "Pa") = 99999.99999999999) annotation(
        Placement(visible = true, transformation(origin = {-10, 4}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
      connect(ramp.y, sourceMassFlow.in_w0) annotation(
        Line(points = {{-100, 54}, {-74, 54}, {-74, 32}}, color = {0, 0, 127}));
  ...
  package Components
    ...
    partial model BaseHX
      ...
      parameter SI.Length L = 0.8 "Length of the HX" annotation(
        Dialog(group = "Dimension"));
      parameter SI.Length W = 0.3 "Width of the HX" annotation(
        Dialog(group = "Dimension"));
      parameter SI.Length D = N * (t_f + t_w) "Depth of the HX, dependent" annotation(
        Dialog(group = "Dimension"));
      parameter SI.Length t_f = 0.0022 "Width of the HX" annotation(
        Dialog(group = "Dimension"));
      parameter SI.Length t_w = 0.0005 "Width of the HX" annotation(
        Dialog(group = "Dimension"));
      final parameter SI.Length dx = L / n annotation(
        Dialog(group = "Dimension"));
      final parameter SI.Volume dV_f = W * dx * t_f annotation(
        Dialog(group = "Dimension"));
      final parameter SI.Volume dV_w = W * dx * t_w annotation(
        Dialog(group = "Dimension"));
      parameter SI.CoefficientOfHeatTransfer h_t = 400 "Heat transfer coefficient";
      parameter SI.SpecificHeatCapacity c_w = 15e3;
      parameter SI.Density rho_w = 7500;
      SI.Temperature Th[n](start = linspace(Tc_in, Th_in, n), each displayUnit = "K") "Hot side temperature";
      SI.Temperature Tw[n](start = linspace(Tc_in, Th_in, n), each displayUnit = "K") "Wall temperature";
      SI.Temperature Tc[n](start = linspace(Tc_in, Th_in, n), each displayUnit = "K") "Cold side temperature";
      SI.Temperature dT[n](each displayUnit = "K") "Temperature difference";
      ...
      SI.Power Qc[n] "Heat transfer at slice i";
      SI.Power Qh[n] "Heat transfer at slice i";
      parameter SI.Power Q_nom = (Th_in - Tc_in) / 2 annotation(
      ...
      parameter Medium.SpecificEnthalpy hh_in_nom "Hot side inlet enthalpy" annotation(
        Dialog(tab = "Initialization"));
      parameter Medium.SpecificEnthalpy hc_in_nom "Cold side inlet enthalpy" annotation(
        Dialog(tab = "Initialization"));
      SI.Pressure p_hot(start = 1e5) "Hot side Pressure";
      SI.Pressure p_cold(start = 1e5) "Cold side pressure";
      parameter SI.Pressure p_hot_nom = 1e5 "Hot side Pressure" annotation(
        Dialog(tab = "Initialization"));
      parameter SI.Pressure p_cold_nom = 1e5 "Cold side pressure" annotation(
        Dialog(tab = "Initialization"));
      final parameter SI.Temperature Tc_in = Medium.temperature_ph(p_cold_nom, hc_in_nom) annotation(
        Dialog(tab = "Initialization"));
      final parameter SI.Temperature Th_in = Medium.temperature_ph(p_hot_nom, hh_in_nom) annotation(
        Dialog(tab = "Initialization"));
    equation
    //Heat Transfer
      Qc = W * dx * h_t * (Tw - Tc);
    ...
    equation
//Energy equations
      Qc[2:n] * (N - 1) = wc * (hc[2:n] - hc[1:n - 1]) "cold fluid";
      Qh[1:n] = Qc[1:n] "wall";
      Qh[1:n - 1] * (N - 1) = wh * (hh[2:n] - hh[1:n - 1]) "hot fluid";
//Mass flow rates
      cold_outlet.m_flow + cold_inlet.m_flow = 0;
      wc = cold_inlet.m_flow;
...
//Pressure at the flanges
    ...
    initial equation
      Qc[2:n] * (N - 1) = wc * (hc[2:n] - hc[1:n - 1]) "cold fluid";
      Qc[1:n] = Qh[1:n] "wall";
      Qh[1:n - 1] * (N - 1) = wh * (hh[2:n] - hh[1:n - 1]) "cold fluid";
    equation
//Energy equtions
      for i in 1:n - 1 loop
        (N - 1) * Qc[i + 1] + wc * (hc[i] - hc[i + 1]) = state_c[i + 1].d * dV_f * div(N, 2) * der(hc[i + 1]) "cold fluid";
        (-(N - 1) * Qh[i]) + wh * (hh[i + 1] - hh[i]) = state_h[i].d * dV_f * div(N + 1, 2) * der(hh[i]) "hot fluid";
      end for;
      Qh[1:n] - Qc[1:n] = c_w * dV_w * rho_w * der(Tw[1:n]);
//Mass flow rates
      cold_outlet.m_flow + cold_inlet.m_flow = 0;
      wc = cold_inlet.m_flow;
      hot_inlet.m_flow + hot_outlet.m_flow = 0;
...
//Pressure at the flanges
    ...
    model StodolaTurbine "Steam turbine"
      ...
      ThermoPower.Water.FlangeB outlet(redeclare package Medium = Medium, m_flow(max = 0)) annotation(
...
// steam boundary conditions and inlet steam properties
      ...
      w = homotopy(Kt * theta * sqrt(pin * rho) * ThermoPower.Functions.sqrtReg(1 - (1 / PR) ^ 2), theta * wnom / pnom * pin) "Stodola's law";
...
// The next equation is provided to close the balance but never actually used
      ...
      annotation(
        Icon(graphics = {Polygon(fillPattern = FillPattern.Solid, lineThickness = 0.5, points = {{-28, 76}, {-28, 28}, {-22, 28}, {-22, 82}, {-60, 82}, {-60, 76}, {-28, 76}}), Polygon(fillPattern = FillPattern.Solid, lineThickness = 0.5, points = {{26, 56}, {32, 56}, {32, 76}, {60, 76}, {60, 82}, {26, 82}, {26, 56}}), Rectangle(fillColor = {160, 160, 164}, fillPattern = FillPattern.Sphere, extent = {{-60, 8}, {60, -8}}), Polygon(fillPattern = FillPattern.Solid, lineThickness = 0.5, points = {{-28, 28}, {-28, -26}, {32, -60}, {32, 60}, {-28, 28}}), Text(extent = {{-130, -60}, {128, -100}}, textString = "%name")}),
        Diagram(graphics),
...
//Boundary conditions
    ...
    model boiler_and_turbine_and_condenser_test
      ...
      Components.DynamicHX dynamicHX(N = 100, Q_nom = 1, h_t = 100000, n = 50) annotation(
        Placement(visible = true, transformation(origin = {-6, 50}, extent = {{-10, 10}, {10, -10}}, rotation = 0)));
      inner ThermoPower.System system annotation(
        Placement(visible = true, transformation(origin = {86, 86}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Modelica.Mechanics.Rotational.Sources.ConstantSpeed constantSpeed(w_fixed = 3000 / 60 * 3.14159) annotation(
      ...
      Components.DynamicHX dynamicHX1(L = 5, N = 100, Q_nom = 1, h_t = 100000, n = 50) annotation(
